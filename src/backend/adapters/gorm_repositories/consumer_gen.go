...
type ConsumerRepository struct{ db *gorm.DB }

func (r *GORMRepository) Consumer() repositories.ConsumerRepository {
	return &ConsumerRepository{r.db}
}

func (r *ConsumerRepository) Get(id string) (entities.Consumer, error) {
	obj := Consumer{}
	ent := entities.NewConsumer()
...
func (r *ConsumerRepository) orderQuery(query *gorm.DB, order *repositories.ConsumerOrder) (*gorm.DB, error) {
	if order == nil {
		return query, nil
	}

	column, exists := consumerFieldToColumn[order.By]
	if !exists {
...
func (r *ConsumerRepository) sliceQuery(query *gorm.DB, order *repositories.ConsumerOrder, slice *repositories.ForwardSlice) (*gorm.DB, error) {
	if slice == nil {
		return query, nil
	}
...
func (r *ConsumerRepository) Find(params *repositories.ConsumerFindParams) ([]entities.Consumer, error) {
	query := r.db.Model(&Consumer{})
...
func (r *ConsumerRepository) Create(ent *entities.Consumer) error {
	obj := Consumer{}
	obj.From(ent)
...
func (r *ConsumerRepository) Update(ent *entities.Consumer) error {
	obj := Consumer{}
	obj.From(ent)
