...
func (interactor *AuctionInteractor) Create(
	...
	room, err := interactor.roomRepo.Get(params.RoomID)
	if err != nil {
		return auction, errors.Wrap(err, "room repo get")
	}

	table, err := interactor.tableRepo.Get(params.BidStepTableID)
	if err != nil {
		return auction, errors.Wrap(err, "table repo get")
	}

	_, err = interactor.productRepo.Update(params.ProductID, func(product *entities.Product) error {
		...
		err := interactor.auctionRepo.Create(&auction)
		return errors.Wrap(err, "auction repo create")
	})

	if err != nil {
		return auction, errors.Wrap(err, "product repo update")
	}
